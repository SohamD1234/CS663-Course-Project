function color_encode(Q, ps, src_filename)
% encode.m
...
if size(im, 3) == 3
    ...
    % Apply im_encode to each channel
    im_encode(Y, src_filename);
    im_encode(Cb, src_filename);
    im_encode(Cr, src_filename);
else
    im_encode(double(im), src_filename);
end


    function im_encode(im, src_filename)
    ...
    % Define the standard 8x8 quantization matrix for quality factor = 50
            ...
            for patch_idx = 1:num_patches
                ...
                for i = 1 + (patch_idx-1) * coeffs_per_patch : patch_idx * coeffs_per_patch
                    ...
                    if coeff == 0
                        ...
                        assert(isequal(size(size_bits_bin), [1, 8]), 'The huffman code is %d long', size_bits);
                        run_length = 0;
                    end
                end
        ...
        function bitstream = dict2Bitstream(dict)
            ...
            for i = 1:size(dict, 1)
                ...
                if key >= 0
                    key_bits = dec2bin(key, 12) - '0';  % Use 12 bits to store the key
                else
                    key_bits = [1, dec2bin(-key, 11) - '0'];
                end
                
                % Store the length of the Huffman code as a 4-bit binary number

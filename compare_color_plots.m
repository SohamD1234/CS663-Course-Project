    ...
    mkdir(folderName);
        ...
        for j = 1:length(files)
            ...
            for k = 1:length(qValues) 
                ...
                % call color_encode with input q, file
            ...
            title(sprintf("Color JPEG: RMSE vs. BPP ps=%s for %s ",num2str(ps), filename), 'FontSize', 4);
            ...
            set(gca, 'FontSize', 7); 
            legend('JPEG Compression', 'MAT Compression', 'Location', 'Best');
            plotfile = sprintf('%s_ps%s.jpg',filename,num2str(ps));
            saveas(gcf, fullfile('compare_color_plots', plotfile)); 
